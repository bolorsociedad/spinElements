...
function Ball(l, t, i) {
	...
	this.a = function() {
		...
		document.getElementById("bl").innerHTML+="<div class='b b" + this.c + "' id='i" + this.i + "'></div>";
		//this.el=document.getElementById("i" + this.i);
		this.u();
		v=[200-this.l-10, 300-this.t-10];
		this.v=[v[0]/(Math.sqrt(v[0]*v[0]+v[1]*v[1])), v[1]/(Math.sqrt(v[0]*v[0]+v[1]*v[1]))];
	}
...
function wi() {
	pun+=mul;
	    					...
	    					if (str%10==0) {
	    						...
	    						setTimeout(function() { document.getElementById("ml").style.visibility="hidden"; }, 2000);
	    					}
}
...
function l() {
	if (end==0) {
	    ...
	    for (j=0;j<b.length;j++) {
	    	...
	    	b[j].t += b[j].v[1];
	    	b[j].u();
	    	if ((190-b[j].l)*(190-b[j].l) + (280-b[j].t)*(280-b[j].t)<3600) {
	    		// Collision
